...
```{r}
merge.obj <- RunPCA(merge.obj, assay = "Spatial", npcs = 10, features = rownames(merge.obj), pcs.print = 1:10)

pca_by_gene <- print(merge.obj[["pca"]], dims = 1:15, features = 20)
write(pca_by_gene, file = "pca_by_gene.txt")

stdev <- merge.obj@reductions$pca@stdev
...
#Let's also examine the number of spots per region. If a region has <300 spots, we should exclude that region. 
spot_counts <- table(merge.obj@meta.data$Spatial_snn_res.0.075)

#    0    1    2    3    4    5 
# 7544 1811 6289 3402  270 1020 

#Region 4 is the ventricular system, so we will omit that region from the analysis. 
