...
#This looks better. Let's proceed to clustering. 
...
```{r}
merge.obj <- FindNeighbors(merge.obj, reduction = "harmony", dims = 1:10)
...
Original issues: 
1. Only 2 images viewable. I need to download the adult images. (Adult images downloaded but not successfully loading - still investigating)
...
slm_dim_0.1 <- SpatialDimPlot(merge.obj_slm, group.by = "Spatial_snn_res.0.1", ncol = 4,label = F, image.alpha = 1, alpha = 1, pt.size.factor = 740)

#SpatialDimPlot of Louvian method
louv_dim_0.1 <- SpatialDimPlot(merge.obj, group.by = "Spatial_snn_res.0.1", ncol = 2,label = F, image.alpha = 1, alpha = 1, pt.size.factor = 740)
