...
#This looks great - if you want to see each plot. If not, use the code below: 
```{r}
...
Using visual analysis of the above, 10, 12, 15, or 19 could be used as breakpoints. Clustering results were poor with 10 PC's, so let's proceed with 15 PC's. 
...
merge.obj <- FindClusters(merge.obj, resolution = 0.093, algorithm = 3)
#clustree(merge.obj)
...
dim_0.093 <- SpatialDimPlot(merge.obj, group.by = "Spatial_snn_res.0.093", ncol = 2,label = F, image.alpha = 1, alpha = 1, pt.size.factor = 3)
#ggsave("SpatialRes0.15.png")

save(merge.obj, file = "MD_merge.obj.Rdata")
#saves current output to speed up future work

#gsave("~/Xie/MouseDevelopment/Figures/SpatialDmPlot.cluster.tiff", device='tiff', dpi=500)
```
